...
#include"SudokuVer2.h"
	...
	/*			system("clear");
		...
		void Sudoku::InputPossibility(){
			int ps_t=-1;
			ps_t = nextBlank(-1);
			while(ps_t<sudokuSize){
			GetPossibility(ps_t);
			ps_t = nextBlank(ps_t);
			}
		}

		void Sudoku::SetPossibility(int ps_t){
			int temp = possibility[ps_t][0];
			for(int i=0;i<9;i++)
				possibility[ps_t][i]=possibility[ps_t][i+1];
			possibility[ps_t][9]=temp;
			while(possibility[ps_t][0] == 0 && possibility[ps_t][1] == 0){
				temp = possibility[ps_t][0];
				for(int i=0;i<9;i++){
					possibility[ps_t][i]=possibility[ps_t][i+1];
		...
		void Sudoku::GetPossibility(int ps_t){
			int check_arr[12];
			int unity[9];
			int row_start;
			int col_start;
